...
from authority.models import hasMonitorAuth
...
def index(request): 
    ...
    # 是否展示监控项管理节点
    monitor_read_flag = hasMonitorAuth(auth_set,"read")      
    user = getUser(request.user.id)  
    return render_to_response('common/index.html',{'show_type_dict':show_type_dict, \
        'app_list':app_list,'user':user,'auth_set':auth_set, 'monitor_read_flag':monitor_read_flag}) 
