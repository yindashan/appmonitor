...
def selecteddel(request):
    ...
    if ids:
        ...
        role_name_list = ""
        for item in id_list:
            role = Role.objects.get(id=int(item))
            role_name_list +=(" " + role.name)
            # 删除角色和人的关联关系
            role.users.clear()
            # 删除角色和权限的关联关系
            role.permissions.clear()
            # 删除此角色
            role.delete()
        Log(username=request.user.username,log_type=2,content="execute selecteddel role " + role_name_list + " success!", level=1).save()
    return HttpResponse(simplejson.dumps({"statusCode":200,"url": "/role/index", "message":u'删除成功'}), mimetype='application/json')
